...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[:, :, even_ind] = torch.tensor([[math.sin(i*pow(1e4, -j/embed_dim)) for j in even_ind] for i in index])

        pe[:, :, odd_ind] = torch.tensor([[math.cos(i*pow(1e4, -(j - 1)/embed_dim)) for j in odd_ind] for i in index])
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.n_head
        q = self.query(query).view(N, S, H, E//H).transpose(1, 2) # (N, H, S, E/H)
        # print(q.shape). #[1, 2, 3, 4]
        k = self.key(key).view(N, T, H, E//H).transpose(1, 2) # (N, H, T, E/H)

        # print(k.shape) #[1, 2, 3, 4]
        v = self.value(value).view(N, T, H, E//H).transpose(1, 2) # (N, H, T, E/H)

        e = q.matmul(k.transpose(2, 3)) / torch.sqrt(torch.Tensor([E / H])) #(N, H, S, T)
        if attn_mask is not None:
          e = e.masked_fill(attn_mask == False, -float('inf'))
        # print(e)
        a_probs = F.softmax(e, dim = 3)
        a = self.attn_drop(a_probs) # -1))
        output = a.matmul(v) #(N, H, S, T) * (N, H, T, E/H) = (N, H, S, E/H)
